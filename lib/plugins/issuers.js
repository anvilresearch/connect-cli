...
function registerIssuers (cli, options, done) {
  ...
  cli.issuers = {
      ...
      issuerSessions.forEach(function (issuerSessionPath) {
        if (path.extname(issuerSessionPath) === '.json') {
          try {
            ...
            if (issuers[name]) {
              issuers[name].session = cli.fs.readJSON(
    ...
    save: function (issuer, name) {
      ...
      try {
        mkdirp.sync(configPath)
        cli.fs.writeYAML(path.join(configPath, name + '.yaml'), issuer, true)
      } catch (e) {
      ...
      mkdirp.sync(sessionPath)
      cli.fs.writeJSON(path.join(sessionPath, name + '.json'), session, false, true)
